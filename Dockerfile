...
LABEL name=tf-market-data-service
...
COPY src ./src
RUN mvn -Dmaven.repo.local=/app/.m3/repository package -DskipTests
...
COPY --from=build-stage /app/target/*.jar market-data-service.jar
...
ENTRYPOINT ["java", "-jar", "market-data-service.jar", "--spring.config.location=file:/app/application.properties", "--spring.profiles.active=default", "--spring.application.name=market-data-service", "--server.port=8085"]